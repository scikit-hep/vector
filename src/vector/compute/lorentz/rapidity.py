...
def xy_z_tau(lib, x, y, z, tau):
    one = t.xy_z_tau(lib, x, y, z, tau)
    two = z
    return 0.5 * lib.log((one + two) / (one - two))


def xy_theta_t(lib, x, y, theta, t):
    one = t
    two = z.xy_theta(lib, x, y, theta)
    return 0.5 * lib.log((one + two) / (one - two))


def xy_theta_tau(lib, x, y, theta, tau):
    one = t.xy_theta_tau(lib, x, y, theta, tau)
    two = z.xy_theta(lib, x, y, theta)
    return 0.5 * lib.log((one + two) / (one - two))
...
def xy_eta_t(lib, x, y, eta, t):
    one = t
    two = z.xy_eta(lib, x, y, eta)
    return 0.5 * lib.log((one + two) / (one - two))


def xy_eta_tau(lib, x, y, eta, tau):
    one = t.xy_eta_tau(lib, x, y, eta, tau)
    two = z.xy_eta(lib, x, y, eta)
    return 0.5 * lib.log((one + two) / (one - two))
...
def rhophi_z_tau(lib, rho, phi, z, tau):
    one = t.rhophi_z_tau(lib, rho, phi, z, tau)
    two = z
    return 0.5 * lib.log((one + two) / (one - two))
...
def rhophi_theta_t(lib, rho, phi, theta, t):
    one = t
    two = z.rhophi_theta(lib, rho, phi, theta)
    return 0.5 * lib.log((one + two) / (one - two))


def rhophi_theta_tau(lib, rho, phi, theta, tau):
    one = t.rhophi_theta_tau(lib, rho, phi, theta, tau)
    two = z.rhophi_theta(lib, rho, phi, theta)
    return 0.5 * lib.log((one + two) / (one - two))
...
def rhophi_eta_t(lib, rho, phi, eta, t):
    one = t
    two = z.rhophi_eta(lib, rho, phi, eta)
    return 0.5 * lib.log((one + two) / (one - two))


def rhophi_eta_tau(lib, rho, phi, eta, tau):
    one = t.rhophi_eta_tau(lib, rho, phi, eta, tau)
    two = z.rhophi_eta(lib, rho, phi, eta)
    return 0.5 * lib.log((one + two) / (one - two))
