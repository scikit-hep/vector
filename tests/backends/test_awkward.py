...
def test_basic():
    ...
    assert isinstance(array, vector._backends.awkward.VectorArray2D)
    ...
    assert isinstance(array[2, 0], vector._backends.awkward.VectorRecord2D)
    assert array[2, 0].rho == 5
    assert array.deltaphi(array).tolist() == [[0], [], [0]]

    array = vector.Array([[{"pt": 1, "phi": 2}], [], [{"pt": 3, "phi": 4}]])
    assert isinstance(array, vector._backends.awkward.MomentumArray2D)
    ...
    assert isinstance(array, vector._backends.awkward.VectorArray3D)
...
def test_rotateZ():
    ...
    out = array.rotateZ(1)
    assert isinstance(out, vector._backends.awkward.MomentumArray2D)
    ...
    assert isinstance(out, vector._backends.awkward.VectorArray2D)
...
def test_projection():
    ...
    assert isinstance(out, vector._backends.awkward.VectorArray4D)
    assert out.tolist() == [
...
def test_add():
    ...
    assert isinstance(one.add(two), vector._backends.awkward.VectorArray2D)
    assert isinstance(two.add(one), vector._backends.awkward.VectorArray2D)
...
def test_zip():
    ...
    assert isinstance(v, vector._backends.awkward.VectorArray2D)
    assert isinstance(v[1], vector._backends.awkward.VectorArray2D)
    assert isinstance(v[1, 0], vector._backends.awkward.VectorRecord2D)
