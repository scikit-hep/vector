...
def test_namedtuples():
    ...
    assert get_x(vector._backends.object.AzimuthalObjectXY(1, 2.2)) == 1
    assert get_x(vector._backends.object.AzimuthalObjectXY(1.1, 2)) == 1.1
...
def test_VectorObjectType():
    ...
    for _ in range(10):
        ...
        if class_refs is None:
            class_refs = sys.getrefcount(vector._backends.object.VectorObject2D)
        assert class_refs + 1 == sys.getrefcount(vector._backends.object.VectorObject2D)
    ...
    obj = vector.obj(x=1, y=2)
    out = one(obj)
    assert isinstance(out, vector._backends.object.VectorObject2D)
    assert out.x == pytest.approx(1)
    assert out.y == pytest.approx(2)

    obj = vector.obj(px=1, py=2)
    out = one(obj)
    assert isinstance(out, vector._backends.object.MomentumObject2D)
    assert out.x == pytest.approx(1)
    assert out.y == pytest.approx(2)

    obj = vector.obj(x=1, y=2, z=3)
    out = one(obj)
    assert isinstance(out, vector._backends.object.VectorObject3D)
    assert out.x == pytest.approx(1)
    assert out.y == pytest.approx(2)
    ...
    out = one(obj)
    assert isinstance(out, vector._backends.object.MomentumObject3D)
    assert out.x == pytest.approx(1)
    assert out.y == pytest.approx(2)
    assert out.z == pytest.approx(3)
    ...
    out = one(obj)
    assert isinstance(out, vector._backends.object.VectorObject4D)
    assert out.x == pytest.approx(1)
    assert out.y == pytest.approx(2)
    assert out.z == pytest.approx(3)
    ...
    out = one(obj)
    assert isinstance(out, vector._backends.object.MomentumObject4D)
    assert out.x == pytest.approx(1)
    assert out.y == pytest.approx(2)
    assert out.z == pytest.approx(3)
    assert out.t == pytest.approx(4)
...
def test_VectorObjectconstructor():
    @numba.njit
    def vector_xy():
        return vector._backends.object.VectorObject2D(
            vector._backends.object.AzimuthalObjectXY(1, 2.2)
        )

    @numba.njit
    def vector_rhophi():
        return vector._backends.object.VectorObject2D(
            vector._backends.object.AzimuthalObjectRhoPhi(1, 2.2)
        )

    @numba.njit
    def momentum_xy():
        return vector._backends.object.MomentumObject2D(
            vector._backends.object.AzimuthalObjectXY(1, 2.2)
        )
    ...
    @numba.njit
    def vector_xyz():
        return vector._backends.object.VectorObject3D(
            vector._backends.object.AzimuthalObjectXY(1, 2.2),
            vector._backends.object.LongitudinalObjectZ(3),
        )

    @numba.njit
    def momentum_xyz():
        return vector._backends.object.MomentumObject3D(
            vector._backends.object.AzimuthalObjectXY(1, 2.2),
            vector._backends.object.LongitudinalObjectZ(3),
        )
    ...
    def vector_rhophitheta():
        return vector._backends.object.VectorObject3D(
            vector._backends.object.AzimuthalObjectRhoPhi(1, 2.2),
            vector._backends.object.LongitudinalObjectTheta(3),
        )
    ...
    @numba.njit
    def vector_xyzt():
        return vector._backends.object.VectorObject4D(
            vector._backends.object.AzimuthalObjectXY(1, 2.2),
            vector._backends.object.LongitudinalObjectZ(3),
    ...
    @numba.njit
    def momentum_xyzt():
        return vector._backends.object.MomentumObject4D(
            vector._backends.object.AzimuthalObjectXY(1, 2.2),
            vector._backends.object.LongitudinalObjectZ(3),
            vector._backends.object.TemporalObjectT(4),
        )
    ...
    def vector_rhophietatau():
        return vector._backends.object.VectorObject4D(
            vector._backends.object.AzimuthalObjectRhoPhi(1, 2.2),
            vector._backends.object.LongitudinalObjectEta(3),
            vector._backends.object.TemporalObjectTau(4),
        )
...
def test_projections():
    ...
    assert isinstance(
        to_Vector2D(vector.obj(x=1.1, y=2.2)), vector._backends.object.VectorObject2D
    )
    assert isinstance(
        to_Vector2D(vector.obj(x=1.1, y=2.2, z=3.3)),
        vector._backends.object.VectorObject2D,
    )
    assert isinstance(
    ...
    assert isinstance(
        ...
        vector._backends.object.MomentumObject2D,
    )
    assert isinstance(
    ...
    assert isinstance(
        to_Vector3D(vector.obj(x=1.1, y=2.2)), vector._backends.object.VectorObject3D
    )
    assert isinstance(
        to_Vector3D(vector.obj(x=1.1, y=2.2, z=3.3)),
        vector._backends.object.VectorObject3D,
    )
    assert isinstance(
    ...
    assert isinstance(
        ...
        vector._backends.object.MomentumObject3D,
    )
    assert isinstance(
    ...
    assert isinstance(
        to_Vector4D(vector.obj(x=1.1, y=2.2)), vector._backends.object.VectorObject4D
    )
    assert isinstance(
        to_Vector4D(vector.obj(x=1.1, y=2.2, z=3.3)),
        vector._backends.object.VectorObject4D,
    )
    assert isinstance(
    ...
    assert isinstance(
        ...
        vector._backends.object.MomentumObject4D,
    )
    assert isinstance(
...
def test_conversions():
    ...
    for v in (
        ...
        if isinstance(v, vector._methods.Momentum):
            assert isinstance(out, vector._backends.object.MomentumObject2D)
        assert isinstance(out.azimuthal, vector._backends.object.AzimuthalObjectXY)
        ...
        if isinstance(v, vector._methods.Momentum):
            assert isinstance(out, vector._backends.object.MomentumObject2D)
        assert isinstance(out.azimuthal, vector._backends.object.AzimuthalObjectRhoPhi)
        assert out.x == pytest.approx(1.1)
        assert out.y == pytest.approx(2.2)
        ...
        if isinstance(v, vector._methods.Momentum):
            assert isinstance(out, vector._backends.object.MomentumObject3D)
        assert isinstance(out.azimuthal, vector._backends.object.AzimuthalObjectXY)
        assert isinstance(out.longitudinal, vector._backends.object.LongitudinalObjectZ)
        assert out.x == pytest.approx(1.1)
        assert out.y == pytest.approx(2.2)
        if isinstance(v, vector._backends.object.VectorObject2D):
        ...
        assert isinstance(out.azimuthal, vector._backends.object.AzimuthalObjectXY)
        assert isinstance(
            out.longitudinal, vector._backends.object.LongitudinalObjectTheta
        )
        assert out.x == pytest.approx(1.1)
        assert out.y == pytest.approx(2.2)
        if isinstance(v, vector._backends.object.VectorObject2D):
        ...
        assert isinstance(out.azimuthal, vector._backends.object.AzimuthalObjectXY)
        assert isinstance(
            out.longitudinal, vector._backends.object.LongitudinalObjectEta
        )
        assert out.x == pytest.approx(1.1)
        assert out.y == pytest.approx(2.2)
        if isinstance(v, vector._backends.object.VectorObject2D):
        ...
        assert isinstance(out.temporal, vector._backends.object.TemporalObjectTau)
        assert out.x == pytest.approx(1.1)
        assert out.y == pytest.approx(2.2)
        if isinstance(v, vector._backends.object.VectorObject2D):
            assert out.eta == pytest.approx(0)
...
def test_method_add():
    ...
    assert not isinstance(out, vector._backends.object.MomentumObject2D)
    assert out.x == pytest.approx(4.1)
    assert out.y == pytest.approx(6.2)
    ...
    assert not isinstance(out, vector._backends.object.MomentumObject3D)
    assert out.x == pytest.approx(4.1)
    assert out.y == pytest.approx(6.2)
    assert out.z == pytest.approx(8.3)
