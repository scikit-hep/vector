...
def test_planar_object():
    ...
    v2 = vector.backends.object_.VectorObject2D(
        vector.backends.object_.AzimuthalObjectXY(5, 12)
    )
    out = v1.add(v2)
...
def test_planar_numpy():
    v1 = vector.backends.numpy_.VectorNumpy2D(
        [(3, 4)],
        dtype=[("x", numpy.float64), ("y", numpy.float64)],
    )
    v2 = vector.backends.numpy_.VectorNumpy2D(
        [(5, 12)],
        dtype=[("x", numpy.float64), ("y", numpy.float64)],
    )
    out = v1.add(v2)
...
def test_spatial_object():
    v1 = vector.backends.object_.VectorObject3D(
        vector.backends.object_.AzimuthalObjectXY(3, 4),
        vector.backends.object_.LongitudinalObjectZ(2),
    )
    v2 = vector.backends.object_.VectorObject3D(
        vector.backends.object_.AzimuthalObjectXY(5, 12),
        vector.backends.object_.LongitudinalObjectZ(4),
    )
    out = v1.add(v2)
    assert out.x == pytest.approx(8)
    assert out.y == pytest.approx(16)
...
def test_lorentz_object():
    v1 = vector.backends.object_.VectorObject4D(
        vector.backends.object_.AzimuthalObjectXY(3, 4),
        vector.backends.object_.LongitudinalObjectZ(2),
        vector.backends.object_.TemporalObjectT(10),
    )
